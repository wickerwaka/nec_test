...
> | item | now |
> |---|---|
> | **X1 / X2** — the fabric leg of the retention intervention | **STILL BLOCKED, AND FOR A NEW REASON.  §59.7.1's blocker is CLOSED**: the core now publishes `AD_OE` (its real pad output enable, user-approved 2026-08-04, one port and one wire on each core), the retention model is re-keyed onto it, the `=== 1'bz` construct is gone, and the model SYNTHESISES — `system_large`'s own dedicated logic registers **27 → 47, exactly +20**, and §59.7.1's isolated construct now gives **20 registers against the z-form's 0**.  The re-key is proved EXACTLY equivalent offline: `tb_sys` base **146/283** and ret **265/283** as before, with all **566** capture records BYTE-IDENTICAL to HEAD's.  **What blocks it now is TIMING**: the retention build is **Fmax 20.25 MHz against a registered ≥ 32**, worst setup **−18.132 ns**, TNS **−11,049.741** — so **NO BITSTREAM WAS FLASHED and the fabric leg was NOT RUN**.  A same-tree control with the macro OFF reproduces FLASH #5 exactly (11,167 ALMs, 6,087 registers, **45.67 MHz**, +9.355 ns), so **the port costs nothing and the whole miss is the 20-flop model**.  **C11's NOT ESTABLISHED stands.**  §69.3-§69.7 |
> | **~~R7, a name-scoped timing exception~~ — REFUTED 2026-08-04 (SM3 sitting 9), REPLACED BY R7′** | ~~"20 flops moved 81 registers out of the collection (2,220 → 2,139)"~~ — **that comparison read two DIFFERENT STAGES of one Quartus flow.**  Stage for stage the collection **GREW**: 2,075 → **2,139** mid-fitter, 2,220 → **2,251** post-fit, with the pre-fitter figure **1,081 in both**.  Nothing escaped the name scope; there is no under-application to fix and **`nec_test.sdc` was NOT edited**.  **R7′, what is actually there**: all **20,000 of 20,000** worst failing paths in the retention build launch from `system_large|c_ready_q` — `always_ff @(posedge clk) c_ready_q <= hb_ready;`, **no enable of any kind** — and land in `v30u_eu` through **55–56 logic levels**.  No CE collection can contain that launch register, so no structural collection can close the miss.  The real fragility: **`READY` reaches the EU's next-state cone single-cycle at 56 levels, and the signed-off build meets timing only because the fitter's physical synthesis happens to break the cone** (`Retimed Register` rows 2,041 → 2,263 between the two builds; synthesis identical apart from the model's +20).  Still **not fixed, and now correctly aimed** — it is an RTL/harness item needing its own pre-registration, not an SDC one.  **§70**, erratum at §69.6 |
