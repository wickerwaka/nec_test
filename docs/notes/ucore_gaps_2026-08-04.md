...
> **FIFTH UPDATE, session SM3 sittings 7-8 (2026-08-04) —
> `ucore_provenance.md` §68-§69 is the current state.**  The X1 row moved
> twice, and the second move replaced its blocker rather than removing it.
>
> | item | now |
> |---|---|
> | **X1 / X2** — the fabric leg of the retention intervention | **RUN.  C11 IS ESTABLISHED AT THE MECHANISM LEVEL (SM3 sitting 12, 2026-08-05, `ucore_provenance.md` §73.8/§73.9) — ⚠ AND ITS REGISTERED NUMERICAL BARS WERE SUPERSEDED, NOT MET (§73.9a, SM3 s13 / Codex concern 3a): §56.3a registered **116 cells / 259 of 283** and what ran is **119 / 265 of 283**, because F43 landed at sitting 6 and moved the offline reference the bar is written RELATIVE to.  §56.3a's registered REFUTATION did not occur in any cell; that is what carries the finding.**  FLASH #6 carries the retention model (`nec_test_ucore.sof 626fb30ebee2…`, Fmax 45.87 MHz, +8.802 ns, TNS 0.000).  `x1_fabric baseline --leg fab_f6` scores **265/283 — the OFFLINE COLUMN EXACTLY**: the **119**-cell INTA class is **119 closed, 0 survivors**, the 18 remaining cells are the SAME 18 named in the pre-registration with the SAME first-divergence coordinate on every one, and scored cell by cell against the `tb_sys ret` leg over all 283 there are **0 PASS/FAIL disagreements and 0 differing coordinates**.  Socket control **49/49**, first light **MATCH 800 ×3**, `use_core=0` chip proof **MATCH 800** after everything, `div_guard` PINNED, **0 transport errors**.  The BEFORE is FLASH #5's **146/283**, and that it is still the right BEFORE is a MEASUREMENT: 566/566 `tb_sys` capture records byte-identical across the IE landing AND the R7′ pass.  **The reading is now a FINDING: at an INTA's T1 the chip's AD pads float and RETAIN the previous data phase, and the 119 fabric-only cells were that and nothing else.**  **X2 (the fabric scorer being stricter than the TB) is CLOSED with it** — on FLASH #6 the two agree on 283 of 283 cells.  **What is NOT explained, and never was: the 18 survivors** — 4 `w0` `busstat` (model-shared, §68.2) and 14 `seg`/`bus` at the top of each sweep's `d` band (§67.3).  They are CORE-OWNED, they are the next HLT-sweep item, and fabric and TB agree on them cell for cell so they are diagnosable offline |
> | **~~R7~~ / ~~R7′~~ — BOTH CLOSED** | R7 was REFUTED at sitting 9 (its "81 registers escaped the CE collection" read two DIFFERENT STAGES of one Quartus flow; stage for stage the collection GREW, and **`nec_test.sdc` was NOT edited then and is NOT edited now**).  **R7′ — `READY` reaching the EU's next-state cone single-cycle at 55–63 logic levels, with timing closure depending on whether the fitter's physical synthesis happened to break it — WAS REAL, and at HEAD it had SWAPPED SIDES**: the DEFAULT build (macro OFF) measured **Fmax 19.42 MHz, worst setup −20.254, TNS −13,129.815**, with **20,000/20,000** failing paths launching from `system_large|c_ready_q` into `v30u_eu` at **62–63 levels** — reproduced to the digit from a DELETED `db`.  **G6 was RED at HEAD and no gate in the tree saw it, because the standing set carries no Quartus leg.**  **CLOSED (SM3 sitting 12) BY ONE MUX**: `eu_rd_edge` is the only carrier of the live READY pin into the EU, and its single consumer seeded `psw_n` at the head of the twelve-position chain; moved onto the `psw` register's own `D` pin, gated by the register-only `row_blocked`.  Control **45.89 MHz**, retention **45.87**, **0** failing paths on both, worst `c_ready_q` path **19 levels**, ladder **ZERO-DELTA at the seed**, no flop added or removed on any entity across six builds.  A first form without `row_blocked` was built, worked, and was **REVERTED by its own pre-registered falsifier** (seed `mc2/2788`).  **`ucore_provenance.md` §73** |
> | **NEW — two rig debts DISCHARGED** | `x1_retention.py` has a **`build()`** with a real dependency set and `capture` calls it (the sixth vacuous-gate instance, §67.7, closed).  `u4_f42_fabric.py` has **`--leg`**, default `core` so every past invocation still means what it meant; FLASH #4's and FLASH #5's fabric records now sit side by side and each reproduces its own number (143/283, 146/283). |
...
|---|---|---|---|---|---|
| **F1** | functional | **8080 / BRKEM execution is STRUCTURALLY UNREACHABLE in the ucore** — the loader never selects `pla3_mode8080` and `ld_page_n` never emits pages 5/6 — while `sim/` DOES implement it | 192 ROM rows + the BRKEM/RETEM/CALLN entries; 116 of 1,028 ROM rows never executed | **ucore** (undeclared RTL-vs-model gap) | render the 8080 page select + entry/exit path, **or** register an explicit scope exclusion so a gate can assert it. **THE EXCLUSION IS NOW REGISTERED — USER DECISION 2026-08-05: "8080 mode should not be tested or considered right now. It is something we will explore in a later campaign."** F1, the class-A PF_LOST population (92 REG seeds, §63.5), all 50 `has_brkem` seeds, and the `t30-brkem` bank (8/62) are DEFERRED-BY-DECISION residue for this campaign — carried in every census as a named exclusion, not worked, not counted against any silicon-match verdict. |
...
| **`TAIL_EXTRA`** — 28 seeds in the ucore's own registered-bank family census | **CLOSED, SM3 SITTING 26 — the family is ZERO in BOTH engines** (`ucore_provenance.md` **§87.A**).  The mechanism is ONE PREDICATE, not the form set: `F` is the OPR interlock, a row that SOURCES `OPR` under an `F` waits for the read that fills it, and at `mod == 3` the decoder issues no operand pre-read — so the wait has no terminator and the EU PARKS (no `HLT` row, no `HALT` status, prefetcher not frozen, queue fills, bus `PASV`).  Swept over 8,192 forms (native + `0F`, every ModR/M `reg`, mod3 and memory) it fires on `62`/`C4`/`C5`/`FE`,`FF` `/3`,`/5` at `mod == 3` and on NOTHING else; `FF /7` at mod3 (2,477 `v20suite` goldens) does not fire because its row WRITES `OPR`, and neither does `8D` LEA — which is why the archived FSM core's `S_HALT` wedge was right on the three and a bug on LEA.  **All 33 model and all 34 ucore seats closed, 0 lost over 3,242, plus 32 `PF_LOST` seeds in each engine that nobody predicted.**  *The superseded text:* **SURVEYED, SM3 SITTING 25 — and it is the ILLEGAL-FORM HALT the tree already measured from the other end.**  29 seeds now; the chip's last bus cycle is a `CODE` fetch on 29/29, the engine's first extra cycle a `MEMR` on 29/29, NO `HALT` status on either side on any of them, and the chip then sits `PASV` with `qs = 0` to the end of the window — its EU has stopped.  The form it stops on carries **`mod == 3` on 27 of 27 resolvable seeds**: `62` CHKIND (8), `C4` LES (6), `FF` `/3`,`/5` (5), `C5` LDS (4), `FE` `/3`,`/5` (4) — every one a form whose microcode must read a MULTI-WORD operand from MEMORY.  `tests/v30/mod3_illegal/metadata.json` (SOCKET, 2026-07-27) already says *"BOUND(62)/LES(C4)/LDS(C5) mod3 HALT on BOTH chip and core"*; the survey ADDS `FE`/`FF` at `/3` and `/5` (9 of the 29) and establishes that the two TIMED engines do not reproduce it.  **NOT LANDED** — the landing changes the SPEC ENGINE's termination behaviour and needs its own pre-registration; registered prediction for whoever takes it: **29 REGISTERED seeds close in BOTH engines and the family goes to 0.** | `ucore_provenance.md` **§86.F**, §58.4 |
