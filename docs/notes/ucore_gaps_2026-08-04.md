...
> | **NEW — R7, a sign-off fragility, and it is not the ucore's** | **`nec_test.sdc`'s CE multicycle is collected BY HIERARCHICAL NAME** (`*|v30u_eu:*|*`, `*|v30u_biu:*|*`) and the fitter DUPLICATES registers across the core boundary.  Adding 20 flops on a capture path moved **81 registers out of the collection** (2,220 → 2,139) and exposed a 48.7 ns `c_ready_q → v30u_eu|wb_kind` path that **does not exist at all in the control**.  So the 45.67 MHz sign-off every ucore bitstream carries depends on where physical synthesis happened to NAME its duplicates.  The SDC's own falsifier covers the exception OVER-applying; this is it UNDER-applying, and no falsifier and no gate in the tree sees it.  **Booked, not fixed** — widening a timing exception after seeing a timing result is the same manoeuvre as choosing a comparator after seeing a score.  §69.6 |
