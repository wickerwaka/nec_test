...
AluResult alu_eval(const Machine& m, const AluLatch& lat) {
    ...
    if (lat.adjust && (op == kAdd || op == kSub)) {
        ...
        uint32_t lo_sum = 0;
        AluResult br = bcd_adjust(m, lat.adjust, op == kSub, av, &lo_sum);
        // The adjust is ONE PASS THROUGH THE ADDER, and the adder is 16 bits
        // wide, exactly as everywhere else (ledger 8.1).  The decimal
        // correction unit sits on the LOW LANE of port B only -- it replaces
        // that lane with `corr`; the HIGH lane of port B goes through raw and
        // the ONE carry chain runs the whole 16 bits.  So the result's high
        // half is `av_hi +/- B_hi +/- carry-out-of-the-low-lane`.
        //
        // MEASURED on the BCD strings (`0F20` / `0F22`), whose ROM rows
        // 02D6 / 02E6 are `ONES -> tmpa / ALU ADD|SUB tmpa`, i.e. B = 0xFFFF:
        // the byte store's COMPANION lane comes out at `av_hi - 1 + carry`
        // (ADD4S) and `av_hi + 1 - borrow` (SUB4S).  A v0.1 census over every
        // divergent store settled both halves: with the high lane held at
        // `av_hi` all 1,262 `0F20` and 1,876 `0F22` divergences were exactly
        // -/+1, and with `+/- B_hi` but no carry the residue was exactly the
        // 778 / 164 stores whose low lane carried.
        uint32_t bhi = bf & 0xFF00u;
        uint32_t ahi = uint32_t(av) & 0xFF00u;
        uint32_t cout = (lo_sum >> 8) & 1u;
        uint32_t rhi = (op == kSub) ? (ahi - bhi - (cout << 8))
                                    : (ahi + bhi + (cout << 8));
        br.value = uint16_t((rhi & 0xFF00u) | (br.value & 0xFFu));
        return br;
    }
